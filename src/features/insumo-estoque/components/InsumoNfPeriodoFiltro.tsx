...
import type { InsumoNfPeriodoPreset } from '@/domain/insumos/insumo-nf-periodo';

type Props = {
  presetAtivo: InsumoNfPeriodoPreset | null;
  de: string;
  ate: string;
  onPreset: (preset: InsumoNfPeriodoPreset) => void;
  onDe: (value: string) => void;
  onAte: (value: string) => void;
};

const PRESETS: { id: InsumoNfPeriodoPreset; label: string }[] = [
  { id: '7dias', label: '7 dias' },
  { id: '30dias', label: '30 dias' },
];

export default function InsumoNfPeriodoFiltro({
  presetAtivo,
  de,
  ate,
  onPreset,
  ...
  return (
    <div className="space-y-3 rounded-2xl bg-stone-50 p-3">
      <div
        className="grid grid-cols-2 gap-1 rounded-xl bg-white p-1 shadow-sm ring-1 ring-stone-200"
        role="group"

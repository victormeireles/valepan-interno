...
const { painelFermentacaoService } = await import('./painel-fermentacao-service');
...
function makeLote(id: string, assadeiras: number): FermentacaoLoteRecord {
  return {
    id,
...
describe('PainelFermentacaoService.getPainelForDate', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('retorna ordens vazias quando não há ordens', async () => {
    ...
    const result = await painelFermentacaoService.getPainelForDate('2026-06-17');
...
describe('PainelFermentacaoService.getCargaCompleta', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    findByIdsTipos.mockResolvedValue([
      {
  ...
  it('retorna ordens e snapshots de comparação', async () => {
    ...
    expect(result.date).toBe('2026-06-17');
    expect(result.ultimaDataComDados).toBe('2026-06-17');
    expect(result.ordens).toHaveLength(1);
    ...
    expect(result.comparacaoSemana.items).toEqual([
      { assadeiras: 0, pedidoAssadeiras: 10 },

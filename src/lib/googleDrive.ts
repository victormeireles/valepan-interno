 ...
 */
function generateFileName(rowId: number, photoType: 'pacote' | 'etiqueta' | 'pallet' | 'forno' | 'fermentacao'): string {
...
async function ensureDateFolder(): Promise<string> {
  ...
  for (const folderName of pathParts) {
    try {
      ...
      console.error(`Erro ao criar/buscar pasta ${folderName}:`, error);
...
export async function uploadPhotoToDrive(
  ...
  mimeType: string,
  rowId: number,
  photoType: 'pacote' | 'etiqueta' | 'pallet' | 'forno' | 'fermentacao'
...
export async function deletePhotoFromDrive(photoId: string): Promise<void> {
  try {
    ...
    console.error('Erro ao deletar foto do Google Drive:', error);
...
export async function listPhotosInPeriod(startDate: Date, endDate: Date): Promise<PhotoInfo[]> {
  try {
    ...
    console.error('Erro ao listar fotos do período:', error);
    return [];
  }
}

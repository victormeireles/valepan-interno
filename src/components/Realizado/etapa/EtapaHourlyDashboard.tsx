...
type EtapaHourlyDashboardProps = {
  selectedDate: string;
  unitLabel: string;
  unitName: string;
...
export default function EtapaHourlyDashboard({
  selectedDate,
  unitLabel,
  unitName,
  items,
      ...
      >
        ...
        {!comparisonPrev && hoursUnion.length > 0 && (
          <p className="px-4 pb-3 text-[13px] leading-snug text-text-muted sm:text-base">
            Não foi encontrado dia anterior com ordens (até 14 dias atrás).
          </p>
        )}
        {semHorario > 0 && (

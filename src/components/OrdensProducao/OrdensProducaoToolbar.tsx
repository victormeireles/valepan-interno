...
type OrdensProducaoToolbarProps = {
  ...
  onImport: () => void;
  onNewOrder: () => void;
  estimativaIndisponivel?: boolean;
};
  ...
  return (
    <header className={ordensProducaoToolbarClass}>
      ...
      {estimativaIndisponivel ? (
        <p className="mt-3 rounded-[9px] border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
          Estimativa indisponível: cadastre produtividade do mês.
        </p>
      ) : null}
    </header>
  );

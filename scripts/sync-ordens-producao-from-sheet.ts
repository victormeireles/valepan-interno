/**
 * Sincroniza ordens de produção (meta A–J) da planilha para Supabase.
 *
 * Janela default: data_producao (coluna A) de hoje −3 até hoje +3 (fuso BR).
 *
 * Uso:
 *   npx tsx scripts/sync-ordens-producao-from-sheet.ts [--dry-run] [--from=YYYY-MM-DD] [--until=YYYY-MM-DD]
 */
...
async function main() {
  ...
  console.log('[sync-ordens-producao] concluído');
